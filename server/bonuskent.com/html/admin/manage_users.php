...
//Now that we've created such a nice heading for our html table, lets create a heading for our csv table
    $csv_hdr = "User Name,Password,First Name,Last Name,Email,Role,Date Enrolled,Date Updated,Gender,Date Of Birth,Education,City,Network,Status,POI,Facebook Id";
	$csv_output = "";
    ...
    <div id="container2">
      ...
      <div class="row-fluid">
        <div class="box gradient">
          <div class="title">
            <h3>
            <i class=" icon-bar-chart"></i><span>Users table</span>
            </h3>
          </div>
          <!-- End .title -->
...
<?php
	$result = mysql_query("SELECT * FROM user where role = 'enduser' ");
	...
	while($row = mysql_fetch_array($result)){
          ...
          ";
		   ...
		   $csv_output .= $row['password'] . ", ";
		   $csv_output .= $row['firstname'] . ", ";
		   $csv_output .= $row['lastname'] . ", ";
		   $csv_output .= $row['email'] . ", ";
		   $csv_output .= $row['role'] . ", ";
		   $csv_output .= $row['date_created'] . ", ";
		   $csv_output .= $row['date_updated'] . ", ";
		   $csv_output .= $row['gender'] . ", ";
		   $csv_output .= $row['date_of_birth'] . ", ";
		   $csv_output .= $row['education'] . ", ";
		   $csv_output .= $row['city'] . ", ";
		   $csv_output .= $row['network'] . ", ";
     ...
     echo "    <td class='ms'>
                <div class='btn-group1'>
                  <a class='btn btn-small' rel='tooltip' data-placement='left' data-original-title=' edit ' href=\"edituser.php?userId=" . $row['id'] . "\" ' ><i class='gicon-edit'></i></a>                  
                  
				  <a class='btn btn-inverse btn-small' rel='tooltip' data-placement='bottom' data-original-title='Remove' data-toggle='modal' href='#myModal' onClick='return setDeleteUser(".$row['id'].") ;' >
...
?>
        ...
        <br />
		<button class="btn btn-primary" onClick="exportToCSV()" style="margin-left:30px;">Export To CSV</button>
                  
          <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<form id='deleteUserForm' name="deleteUserForm" method='post' action='ajaxdeleteUser.php'>
            	<input type="hidden" name="userIdToDelete" id="userIdToDelete" value="">
...
<script type="text/javascript" src="scripts/common.js"></script>
	...
	<div style="display:none">
        <form name="exportToCSVForm" id="exportToCSVForm" action="exportToCSV.php" method="post">
